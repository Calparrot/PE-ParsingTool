...
Core::Diagnostic value_mismatch(
	Core::Severity severity, const std::string& field, const std::string& description,
    uint32_t expected, uint32_t actual, uint64_t offset
) {
    Core::Diagnostic d;
...
Core::Diagnostic invalid_value(
    Core::Severity severity, const std::string& field, const std::string& description,
    uint32_t actual, uint64_t offset, bool is_signature
) {
    Core::Diagnostic d;
    d.object = is_signature ? Core::Object::SIGNATURE : Core::Object::FIELD;
    d.severity = severity;
    d.category = Core::DiagCategory::INVALID_VALUE;
    d.field_name = field;
    d.description = description;
    d.actual_value = actual;
    d.offset = offset;
...
Core::Diagnostic excursion_anomaly(
    Core::Severity severity, const std::string& field, 
    const std::string& description, uint64_t address
) {
    Core::Diagnostic d;
...
Core::Diagnostic abnormal_length(
    Core::Severity severity, const std::string& description,
    uint32_t actual, uint64_t offset
) {
    Core::Diagnostic d;
...
Core::Diagnostic indexed_issue(
    Core::Severity severity, const std::string& description,
    size_t index, const std::string& info1, uint64_t offset
) {
    Core::Diagnostic d;
    d.object = Core::Object::STRUCTURE_ADDRESS;
    d.severity = severity;
...
Core::Diagnostic relationship_issue(
    Core::Severity severity, const std::string& field, const std::string& description,
    const std::string& compared_field, const std::string& compared_description,
    const std::string& info1, uint64_t offset
) {
    Core::Diagnostic d;
    d.object = Core::Object::FIELD;
...
// 创建额外信息相关的诊断
// {info2}
Core::Diagnostic additional_information( const std::string& info2, uint64_t offset){
    Core::Diagnostic d;
    d.object = Core::Object::STRUCTURE;
